...
export default function WhyUsSection() {
  return (
    <section id="about" className="relative w-full h-auto lg:min-h-[100dvh] flex flex-col justify-center py-6 lg:py-32 bg-white dark:bg-[#0a0a0a] transition-colors duration-500 overflow-hidden">
      <div className="max-w-[1440px] mx-auto w-full px-4 lg:px-12 flex flex-col gap-6 lg:gap-24 relative z-10 h-full">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-stretch flex-grow">
          ...
          <div className="w-full lg:w-[45%] flex flex-col items-start pr-0 lg:pr-12">
            <p className="text-gray-500 dark:text-gray-400 text-[8px] md:text-xs tracking-[0.2em] uppercase font-bold mb-2 lg:mb-8 transition-colors">
              About Lawyer Panel
            </p>
            <h2 className="text-3xl md:text-5xl lg:text-[64px] font-serif leading-[1.05] mb-2 lg:mb-8 text-gray-900 dark:text-white tracking-tight transition-colors">
              A Safer Tomorrow.
            </h2>
            <p className="text-gray-600 dark:text-gray-300 text-[10px] md:text-base leading-[1.5] lg:leading-[1.6] mb-4 lg:mb-10 max-w-[480px] font-light transition-colors">
            ...
            {features.map((item, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col items-start">
                  <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full border border-gray-200 dark:border-white/20 flex items-center justify-center mb-2 lg:mb-5 text-gray-900 dark:text-white hover:border-[#00b33c] hover:text-[#00b33c] transition-colors duration-300 cursor-default">
                    <Icon size={12} strokeWidth={1.5} className="lg:w-[14px] lg:h-[14px]"/>
                  </div>
                  <h3 className="text-[9px] md:text-xs font-bold text-gray-900 dark:text-white tracking-widest uppercase mb-1">{item.title}</h3>
                  <p className="text-[9px] md:text-xs text-gray-500 dark:text-gray-400 font-light leading-relaxed">{item.desc}</p>
                </div>
              )
            })}
          </div>
          ...
          <div className="w-full lg:w-[20%] flex flex-row lg:flex-col justify-between items-center lg:items-stretch bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/10 p-4 lg:p-8 group cursor-pointer hover:border-[#00b33c]/50 dark:hover:border-[#00b33c]/50 transition-all duration-500">
            ...
            <div className="flex items-center justify-between lg:border-t border-gray-200 dark:border-white/10 lg:pt-6 gap-4">
              ...
              <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full border border-gray-200 dark:border-white/20 flex items-center justify-center text-gray-400 dark:text-gray-500 group-hover:bg-[#00b33c] group-hover:border-[#00b33c] group-hover:text-white transition-all duration-300">
                <ArrowRight size={12} />
              </div>
            </div>
          </div>
