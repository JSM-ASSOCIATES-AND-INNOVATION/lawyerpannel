...
export default function Button({ variant = 'primary', className, children, ...props }) {
  const baseStyle = "relative inline-flex items-center justify-center font-bold tracking-widest uppercase transition-all duration-300 overflow-hidden text-sm cursor-pointer";
      ...
      <header className="w-full bg-white/90 dark:bg-[#1a1818]/90 backdrop-blur-md border-b border-gray-100 dark:border-white/10 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button className="hidden md:flex !py-0 h-10 px-6">Consult Now</Button>
          </div>
        </div>
      </header>
    </div>
...
if(!cssContent.includes('background-color: #232020')) {
  cssContent = cssContent.replace('@tailwind utilities;', '@tailwind utilities;' + injectCss);
